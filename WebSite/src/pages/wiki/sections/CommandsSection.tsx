...
const commands: CommandDoc[] = [
  ...
  },
  {
    name: '/land',
    scope: 'MEMBER',
    summary: '土地保護コアの情報表示・境界線デバッグを行います。',
    usage: '/land <debug|info>',
    subCommands: [
  ...
  },
  {
    name: '/sit',
    scope: 'ANY',
    summary: '現在地に座ります。もう一度実行するか降車キーを押すと立ち上がります。',
    usage: '/sit',
    notes: ['足元に座れるブロックが必要です。乗り物に乗っている場合は先に降ります。'],
  },
  {
    name: '/pet',
    scope: 'ANY',
    summary: 'Petにできる友好モブと、専用「なかよし素材」の合成レシピを確認します。',
    usage: '/pet <list|recipe モブ名>',
    subCommands: [
      { name: '/pet list', text: 'Pet化に対応しているモブを一覧表示。' },
      { name: '/pet recipe <モブ名>', text: 'そのモブ用のなかよし素材に必要な2アイテムを表示。' },
    ],
    notes: ['なかよし素材を対象モブへ使った後、スニーク右クリックで「一緒に行動 / 待機 / 放し飼い」を切り替えます。'],
  },
  {
    name: '/hotp',
    scope: 'MEMBER',
  ...
  },
  {
    ...
    summary: 'YouTubeライブのコメントと支援イベントをMinecraftへ中継します。引数なしで使い方Dialogを表示します。',
    usage: '/youtube [配信URL|login|logout|settings|status|stop|setkey APIキー]',
    subCommands: [
      { name: '/youtube', text: '使い方を説明するDialogを表示。' },
      { name: '/youtube settings', text: 'コメント種別、登録者増減通知、BossBarと表示時間を設定するDialogを表示。' },
      { name: '/youtube login', text: 'Googleの認証URLとコードを表示し、チャンネル所有者としてOAuthログイン。' },
      { name: '/youtube logout', text: '保存されたOAuthトークンを削除。' },
      { name: '/youtube <配信URL>', text: '指定したライブ配信との連携を開始。' },
      { name: '/youtube status', text: '現在の連携状態を表示。' },
      { name: '/youtube stop', text: '連携を停止。' },
      { name: '/youtube setkey <APIキー>', text: 'YouTube Data APIキーを保存。コンソールでも利用可能。' },
    ],
    notes: ['OAuthログイン中はAPIキー不要です。公開登録者は名前付き、非公開登録や解除は人数変化として通知します。'],
  },
  {
    name: '/hardmode',
    scope: 'ADMIN_OR_CONSOLE',
    summary: '設定不要の高難易度サバイバル機能を管理します。',
    usage: '/hardmode <list|truecrafter enabled|disabled>',
    subCommands: [
      { name: '/hardmode list', text: 'HardMode機能一覧と現在の状態を表示。' },
      { name: '/hardmode truecrafter enabled', text: '真クラを有効化。' },
      { name: '/hardmode truecrafter disabled', text: '真クラを無効化。' },
    ],
    notes: ['有効・無効の状態はhardmode.ymlへ自動保存されます。追加設定は不要です。'],
  },
  {
    name: '/invsee',
    scope: 'ADMIN',

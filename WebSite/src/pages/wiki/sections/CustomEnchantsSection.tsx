...
const enchantDetails = [
  ...
  { name: '範囲採掘', group: '採掘', target: 'ツルハシなど', text: '中心から渦を巻くように周囲のブロックをまとめて壊すエンチャントです。水平に掘ると掘った地点を最下段にして上方向へ、真下を向いて掘ると従来通り真下へ掘り進みます。連続採掘でも演出が途切れないよう複数の破壊処理を並列で走らせます。効果はツール設定メニュー (左→右→左→右→右→シフト) で ON/OFF します。' },
  { name: '採掘穴埋め', group: '採掘', target: 'ツルハシ・シャベル・斧', text: 'オフハンドにブロックを持って掘ると、掘った場所をそのブロックで即座に埋めるエンチャントです。ブロックは 1 個ずつ消費されます (クリエイティブは消費なし)。ツール設定メニュー (左→右→左→右→右→シフト) で ON にし、かつオフハンドにブロックを持っているときだけ動作します。' },
...
export function CustomEnchantsSection() {
  return (
    ...
    >
      ...
      <h3>ツール設定メニュー (効果の ON/OFF)</h3>
      <p>
        一部のエンチャントは、常時発動ではなく「ツール設定メニュー」で ON/OFF を切り替えて使います。
        対応エンチャントが付いた道具を手に持ち、<strong>左クリック → 右クリック → 左クリック → 右クリック → 右クリック → シフト</strong>
        の順に手早く操作するとメニューが開きます。手順を長めにし最後をシフトにしているのは、通常操作での誤爆を防ぐためです。
      </p>
      <ul className="wiki-bullets">
        <li>各入力は間隔を空けすぎると手順がリセットされます。テンポよく続けて入力してください。</li>
        <li>メニューには、その道具に<strong>実際に付いているエンチャント</strong>の項目だけが並びます。付いていない効果は表示されません。</li>
        <li>切り替えられる効果が 1 つも無い道具では、手順を行ってもメニューは開きません。</li>
        <li>項目をクリックするたびに ON / OFF が切り替わります。設定はプレイヤーごとに保持されます (サーバー再起動でリセット)。</li>
        <li>現在ここで切り替えられる効果:「範囲採掘」「採掘穴埋め」です (今後増える可能性があります)。</li>
      </ul>

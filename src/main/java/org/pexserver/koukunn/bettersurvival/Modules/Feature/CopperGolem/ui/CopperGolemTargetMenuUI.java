...
public final class CopperGolemTargetMenuUI {

    private CopperGolemTargetMenuUI() {
    }
    ...
    public static void open(Player player, GolemProfile profile, ActionHandler handler) {
        ChestUI.Builder builder = ChestUI.builder()
                .title("保管先設定: " + profile.id())
                .size(54)
                .addButtonAt(45, "§e戻る", Material.ARROW, "")
                .addButtonAt(52, "§c全解除", Material.BARRIER, "§7登録済みの保管先を全削除")
                .addButtonAt(53, "§7閉じる", Material.RED_STAINED_GLASS_PANE, "");

        for (int i = 0; i < 45; i++) {
            if (i < profile.targets().size()) {
                ContainerTarget target = profile.targets().get(i);
                ...
                String lore = "§7" + worldName + " "
                        ...
                        + "\n§7クリックで再設定";
                builder.addButtonAt(i, "§a保管先 " + (i + 1), Material.CHEST, lore);
            } else if (i == profile.targets().size()) {
                builder.addButtonAt(i, "§b保管先を追加", Material.LIME_STAINED_GLASS_PANE,
                        "§7クリック後、対象チェストを左クリック");
            } else {
